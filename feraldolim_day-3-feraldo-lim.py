...
def preprocess (dataset):
    ...
    revenue_10 = processed_dataset.shift(periods = -14)

    revenue_09 = processed_dataset.shift(periods = -13)

    revenue_08 = processed_dataset.shift(periods = -12)

    revenue_07 = processed_dataset.shift(periods = -11)

    revenue_06 = processed_dataset.shift(periods = -10)

    revenue_05 = processed_dataset.shift(periods = -9)

    revenue_04 = processed_dataset.shift(periods = -8)

    revenue_03 = processed_dataset.shift(periods = -7)

    revenue_02 = processed_dataset.shift(periods = -6)

    revenue_01 = processed_dataset.shift(periods = -5)

    

    revenue_0 = processed_dataset.shift(periods = -4)

    revenue_1 = processed_dataset.shift(periods = -3)

    revenue_2 = processed_dataset.shift(periods = -2)

    revenue_3 = processed_dataset.shift(periods = -1)

    revenue_4 = processed_dataset.shift(periods = 0)

#     new = pd.concat([revenue_0,revenue_1.Revenue,revenue_2.Revenue,revenue_3.Revenue,revenue_4.Revenue], axis=1)

#     new.columns = ['Date','today_revenue','d-1_rev','d-2_rev','d-3_rev','d-4_rev']

#     new.drop(new.tail(4).index,inplace=True)

#     processed_dataset = new

#     new.head()

    

    new = pd.concat([revenue_06,revenue_05.Revenue

                     ,revenue_04.Revenue,revenue_03.Revenue,revenue_02.Revenue,revenue_01.Revenue,

                     revenue_0.Revenue,revenue_1.Revenue,revenue_2.Revenue,revenue_3.Revenue,revenue_4.Revenue], axis=1)

    new.columns = ['Date','today_revenue','d-1_rev','d-2_rev','d-3_rev','d-4_rev','d-5_rev','d-6_rev','d-7_rev'

                  ,'d-8_rev','d-9_rev','d-10_rev']

    new.drop(new.tail(10).index,inplace=True)
...
def split_train_test(dataset, end_of_training_date):
    ...
    training_data = daily_online_revenue.loc['2017-01-05':end_of_training_date]

    testing_data = daily_online_revenue["2017-12-01":]
...
#     y_train = training_data['today_revenue']

#     x_train = training_data[['d-1_rev','d-2_rev','d-3_rev','d-4_rev']]



#     y_test = test_data['today_revenue']

#     x_test = test_data[['d-1_rev','d-2_rev','d-3_rev','d-4_rev']]

    y_data = train_or_test_data['today_revenue']

    x_data = train_or_test_data[['d-1_rev','d-2_rev','d-3_rev','d-4_rev','d-5_rev','d-6_rev','d-7_rev','d-8_rev','d-9_rev','d-10_rev']]
...
training_data, test_data = split_train_test(daily_online_revenue,end_of_training_date)
...
comparison = pd.DataFrame({"Prediction":result,"Actual":y_test})
...
# date_test = pd.date_range('2017-11-16', periods=29, freq='D')

# date = pd.DataFrame(date_test).rename(columns={0:"Id"})

# date_test

# formatted_result = pd.concat([date,formatted_result.Revenue], axis=1)
...
formatted_result.to_csv("result.csv",index=False)
