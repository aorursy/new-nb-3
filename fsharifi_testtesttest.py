...
xgtest = xgb.DMatrix(test.drop(columns_to_drop, axis=1), label=final_test_preds)
test_preds2 = model.predict(xgtest, ntree_limit=model.best_iteration)
test_preds2 = np.clip(test_preds2, 1, 8)
# apply offsets to test
data = np.vstack((test_preds2, test_preds2, test['Response'].values))
for j in range(num_classes):
    data[1, data[0].astype(int)==j] = data[0, data[0].astype(int)==j] + offsets[j]
final_test_preds = np.round(np.clip(data[1], 1, 8)).astype(int)
