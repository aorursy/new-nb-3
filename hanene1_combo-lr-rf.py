...
for uid, p in id_preds1.items():
    id_preds_combined[uid] = fraction1*np.asarray(id_preds1[uid]) + fraction3*np.asarray(id_preds3[uid])
    
id_preds = id_preds_combined
list(id_preds_combined)
...
sample.head()
sample.to_csv('LR-RF_sub.csv', index=False)
