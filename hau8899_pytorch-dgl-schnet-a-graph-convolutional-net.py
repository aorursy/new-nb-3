...
# DEVICE = torch.device('cuda:0')  # using only GPU here
...
class Molecule_Dataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return {"graph":graph, "target_infos":target_infos}
...
# valid_loader = DataLoader(ds_valid, batch_size=batch_size, shuffle=True, num_workers=4)



net = Atominator()

net.load_state_dict(torch.load('../input/model-schnet/model.pth'))
...
#         graph = ds_train[i]['graph']

#         (j_pairs, y) = ds_train[i]['target_infos']
