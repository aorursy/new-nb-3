...
def best_r():
    ...
    for n in ns:
        ...
        r = r.astype(int)
        df['r'] = r
        data = df[['r','shot_made_flag']]
        data = data.dropna()
        score.append(test_it(data).mean())
        
    plt.figure(figsize=(7, 5))
    plt.plot(ns, score, 'go')
...
df['shot_distance'] = df.apply(lambda row: 28 if row['shot_distance'] > 28 else row['shot_distance'], axis=1)
data = data.dropna()
def test_it(data):
    clf = RandomForestClassifier(n_jobs=-1)  # A super simple classifier
    return -cross_val_score(clf, data.drop('shot_made_flag', 1), data.shot_made_flag,
...
x = df['loc_x'].values
y = df['loc_y'].values
r = np.sqrt(x**2 +y**2) / 20.0
r = r.astype(int)
angle = 5*np.abs(np.arctan(x/y))
...
depth, scores = list(range(1, 20, 1)), []
for i in depth:
    clf = RandomForestClassifier(n_jobs=-1, n_estimators=36, max_depth=i, random_state=2016)
    x = cross_val_score(clf, data.drop('shot_made_flag', 1), data.shot_made_flag,
                              scoring='roc_auc', cv=10)
    scores.append(x)
x = [i for i in depth for j in range(10)]
...
# Train and predict
clf.fit((train.drop('shot_made_flag', 1))[:21000], train.shot_made_flag[:21000])
predictions_train = clf.predict_proba(train.drop('shot_made_flag', 1))
predictions_test = clf.predict_proba(test.drop('shot_id', 1))
...
features_test = pd.DataFrame({'shot_id': test.shot_id,
                           'shot_made_flag': predictions_test[:, 1]})
features_train[['shot_made_flag']].to_csv('features_train.csv', index=False)
features_test[['shot_id','shot_made_flag']].to_csv('features_test.csv', index=False)