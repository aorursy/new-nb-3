...
plt.show()
A, B, C = cv2.findContours(th1.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
A, A.shape
len(B)
C, C.shape
plt.hist(th1.flatten(), bins=100)
index = 5
...
ax0.imshow(cv2.cvtColor(img_dot, cv2.COLOR_BGR2GRAY))

ax1.imshow(img_dot_gray, 'gray')
...
ax0.imshow(img_dot_missing_area)
