...
for patient in patients[:10]:
    ...
    #          the first sliced. +  d.RescaleSlope

    slices[0] = slices[0].pixel_array.astype('float32')*float(slices[0].RescaleSlope)+float(slices[0].RescaleIntercept)

    slices[0][slices[0]==-2000] = 0
...
for patient in patients:
    ...
    for num,each_slice in enumerate(slices[:40]):

        y = fig.add_subplot(5,8,num+1)

        new_img = cv2.resize(np.array(each_slice.pixel_array.astype('int16')),(IMG_PX_SIZE,IMG_PX_SIZE))
