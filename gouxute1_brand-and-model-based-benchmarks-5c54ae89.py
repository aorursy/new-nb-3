...
a = [['one','a',1],['one','b',2],['two','a',3],['two','b',4]]

s = pd.Series(a).reset_index(map(str,range(4)))
...
f(s.unstack(level=-1))
t=gatrain.groupby(['age','gender'])

t.groups

#f((gatrain.groupby(['age','gender'])))
def f(x):
...
#f(gatrain.groupby(['age','gender']).size())

#f(gatrain.groupby(['age','gender']).size().unstack())

df = pd.DataFrame(gatrain.groupby(['age','gender']).size())

df.reset_index(inplace=True)

#f(df)

c = df.pivot(index = 'age',columns = 'gender',values =0).reindex(index=np.arange(gatrain.age.min(), gatrain.age.max()+1)).fillna(0)
...
#c.get_group(c.groups[1])

#f(gatrain.groupby(['age','gender']).size().unstack().reindex(index=np.arange(gatrain.age.min(), gatrain.age.max()+1)))

#print c.head(1)

#.size().unstack().reindex(index=np.arange(gatrain.age.min(), gatrain.age.max()+1)).fillna(0)
