...
#Code to get the file size of each files located in a folder

#@reference: https://www.kaggle.com/jesucristo

#@jesucristo

print('# File sizes')

for f in os.listdir(PATH):  #Path where the data is stored.

    if 'zip' not in f:

        print(f.ljust(30) + str(round(os.path.getsize(PATH + f) / 1000000, 2)) + 'MB')
#function to reduce the memory of the datasets.
...
train_identity['nulls2'] = train_identity.isna().sum(axis=1)

test_identity['nulls2'] = test_identity.isna().sum(axis=1)
emails = {'gmail': 'google', 'att.net': 'att', 'twc.com': 'spectrum', 'scranton.edu': 'other',
          ...
          'outlook.com': 'microsoft', 'mail.com': 'other', 'bellsouth.net': 'other', 'embarqmail.com': 'centurylink',
...
# train_red, _ = reduce_mem_usage1(train)

# test_red, _ = reduce_mem_usage1(test)
# #del unwanted df

# del train, test

# gc.collect()
# #creating X and y variables out of the train_red set.

# X = train_red.drop('isFraud', axis=1)
...
drop_col = ['TransactionDT', 'V300', 'V309', 'V111', 'C3', 'V124', 'V106', 'V125',
...
params = {'learning_rate': 0.009, 'max_depth': 10, 'boosting': 'gbdt', 'objective': 'binary', 'metric':'auc', 'seed': 4, 'num_iterations': 10000, 'early_stopping_round': 100, 'verbose_eval': 200, 'num_leaves': 64, 'feature_fraction': 0.8, 'bagging_fraction': 0.8, 'bagging_freq': 5}
...
#https://www.kaggle.com/artkulak/ieee-fraud-simple-baseline-0-9383-lb <-- Upvote here

sub = simple.Blend(['../input/ieee-fraud-simple-baseline-0-9383-lb/simple_xgboost.csv', 'lgb.csv'], 'TransactionID', 'isFraud', 'blend01.csv')
#https://www.kaggle.com/vaishvik25/ensemble  <-- Upvote here

sub = simple.Blend(['../input/ensemble/stack_minmax_bestbase.csv', 'blend01.csv'], 'TransactionID', 'isFraud', 'blend02.csv')
#https://www.kaggle.com/paulorzp/gmean-of-low-correlation-lb-0-952x   <-- Upvote here

sub = simple.Blend(['../input/gmean-of-low-correlation-lb-0-952x/stack_gmean.csv', 'blend02.csv'], 'TransactionID', 'isFraud', 'blend03.csv')