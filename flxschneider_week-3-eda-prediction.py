...
test['region_id'] = test.apply(lambda x: x['Country_Region'] +"_"+ str(x['Province_State']), axis=1)
...
for c in train.region_id.unique():
    ...
    try: 
        ...
        max_fatal = np.max(df.Fatalities)

        start_date_fatal = df[df.Fatalities > 0.1*max_fatal].head(1).Date.values[0]

        df = df[df.ConfirmedCases > 0.1*max_fatal]
        ...
        forecast = predict.ForecastId.values

        data['ConfirmedCases'].extend([0]*len(forecast))

        data['Fatalities'].extend([0]*len(forecast))

        data['ForecastId'].extend(forecast)

print(counter)
counter
len(train.region_id.unique())
