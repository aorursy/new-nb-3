...
train_file.drop(["Color"], axis=1, inplace=True)
test_file.drop(["Color"], axis=1, inplace=True)
...
def f(x):
    ...
    if 'year' in x:
        return x.split(" ")[0]*50
    return 0
print (conjunto['AgeuponOutcome'].unique())
conjunto['AgeuponOutcome'] = conjunto['AgeuponOutcome'].apply(lambda x: f(x))
print (conjunto['AgeuponOutcome'].unique())
