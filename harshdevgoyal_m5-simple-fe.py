...
prices_df=reduce_mem_usage(prices_df,verbose=True)
prices_df.head()
...
grid_df=grid_df[['store_id','item_id','wm_yr_wk']+MAIN_INDEX]
...
print("{:>20}: {:>8}".format('Full Grid one state',sizeof_fmt(grid_df.memory_usage(index=True).sum())))
...
print("{:>20}: {:>8}".format('Full Grid one store',sizeof_fmt(grid_df.memory_usage(index=True).sum())))
