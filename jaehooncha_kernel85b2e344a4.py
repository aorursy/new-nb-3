...
def confiremd_predicted():

    con_pred_path = os.path.join(mother_path, "confirmed_predicted.pickle")
    ...
    for i in range(len(Result)):
        ...
        if Result[i][0] == 'Korea, South':

            print(Result[i][0])

            print(Result[i][1][-1]-Result[i][1][-2])

            print(np.diff(Result[i][-1]))         
