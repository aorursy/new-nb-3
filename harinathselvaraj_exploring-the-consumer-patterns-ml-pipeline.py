...
p = 0.05 # *** In this case we will use 50% of data set *** #
...
def json_read(df):
    ...
    df = pd.read_csv(data_frame, 
                    ...
                    #nrows=2 #first 2 rows only for testing
                    ) 
...
def missing_values(data):
    ...
    nulls = data.isnull().count().sort_values(ascending = False) #nulls
    percent = (data.isnull().sum() / data.isnull().count() * 100 ).sort_values(ascending = False) #getting the percent and order of null
    df = pd.concat([total, nulls, percent], axis=1, keys=['Total', 'Nulls', 'Percent']) # Concatenating the total and percent
    print("Total Nulls in the Columns and their Percentage: ")
    print (df[~(df['Total'] == 0)]) # Returning only non zero values
    
    print("\n Total of Sales % of Total: ", round(
        (df_train[df_train['totals.transactionRevenue'] != np.nan]['totals.transactionRevenue'].count() 
         / 
         len(df_train['totals.transactionRevenue']) * 100)
        ,4))
    
    return 
# calling the missing values function
missing_values(df_train) 
print(df_train.info()) #gives Not NULL objects, NULL/NOT NULL, datatype (object(string) or integer(int64) or boolean(bool))
...
def date_process(df):
    ...
    df["_month"] = df['date'].dt.day # extracting day
    df["_year"] = df['date'].dt.day # extracting day
#  pd.to_datetime(train_df["date"], format="%Y%m%d") can also be done.
...
def FillingNaValues(df):    # fillna numeric feature
    ...
    df['totals.pageviews'] = df['totals.pageviews'].astype(str).astype(int) # change the datatype to int    
    df['totals.newVisits'] = df['totals.newVisits'].astype(str).astype(int) # change the datatype to int    
    df['totals.bounces'] = df['totals.bounces'].astype(str).astype(int) # change the datatype to int    
    df['totals.transactionRevenue'] = df['totals.transactionRevenue'].astype(str).astype(float) # change the datatype to float    
...
#df_train.totals_pageviews.unique()
FillingNaValues(df_train)
df_train.info()
df_train[1:5]
...
def Normalizing(df):
    ...
    return df 
Normalizing(df_train)
#44954 rows × 57 columns
df_train['device.browserVersion'].value_counts()
...
def knowningData(df, data_type=object, limit=3): #seting the function with df, 
    ...
    for column in n.columns: #initializing the loop
        print(column, ':  ', df[column].unique()[:limit],
              " | ## Total nulls: ", (df[column].isnull().sum() / len(df[column]) * 100),
...
to_drop = ["socialEngagementType",'device.browserVersion', 'device.browserSize', 'device.flashVersion', 'device.language', 'device.mobileDeviceBranding',
           'device.mobileDeviceInfo', 'device.mobileDeviceMarketingName', 'device.mobileDeviceModel', 'device.mobileInputSelector',
           'device.operatingSystemVersion', 'device.screenColors', 'device.screenResolution', 'geoNetwork.cityId', 'geoNetwork.latitude',
           'geoNetwork.longitude','geoNetwork.networkLocation', 'trafficSource.adwordsClickInfo.criteriaParameters',
           'trafficSource.adwordsClickInfo.gclId', 'trafficSource.campaign']
...
dummy_feaures =['channelGrouping', 'device.browser', 'device.deviceCategory', 'geoNetwork.city', 'device.operatingSystem', 
                ...
                'trafficSource.adwordsClickInfo.page', 'trafficSource.adwordsClickInfo.slot', 'trafficSource.campain']
...
pd.crosstab(df_train[country_repayment[0]], df_train[country_repayment[1]], 
            values=df_train["totals.transactionRevenue"], aggfunc=[np.mean]).style.background_gradient(cmap = cm)
