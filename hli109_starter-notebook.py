...
collab_jac10 = collaborative_filter(s_train, s_input, j=30, sim_fn=jaccard_sim, threshold=0.05, k=10)
help(content_filter)
...
collab_cos10 = collaborative_filter(s_train, s_input, sim_fn=cosine_sim, j=41, threshold=0.08, k=k_top)

print("MAP = {0:.3f}".format(mapk_score(s_hidden, get_recs(collab_jac10), k=k_top)))

print("UHR = {0:.3f}".format(uhr_score(s_hidden, get_recs(collab_jac10), k=k_top)))
content_jac10 = content_filter(s_items, s_input, sim_fn=jaccard_sim, threshold=0.2, k=k_top)

print("MAP = {0:.3f}".format(mapk_score(s_hidden, get_recs(content_jac10), k=k_top)))

print("UHR = {0:.3f}".format(uhr_score(s_hidden, get_recs(content_jac10), k=k_top)))
...
udf.query('Hits == 3').head()
show_user_detail(s_input, s_hidden, collab_jac10, uid=2, name_fn=cameo_name)
