...
from tqdm.auto import tqdm    # TQDM is a progress bar library with good support for nested loops and Jupyter/IPython notebooks.

from glob import glob    # https://pymotw.com/2/glob/

import time, gc   # gc : The process by which Python periodically frees and reclaims blocks of memory that no longer are in use is called Garbage Collection.
...
import matplotlib.image as mpimg   # matplotlib.image : The image module supports basic image loading, rescaling and display operations. class matplotlib.image

from keras.preprocessing.image import ImageDataGenerator   # Keras’ImageDataGenerator class to perform data augmentation. https://www.pyimagesearch.com/2019/07/08/keras-imagedatagenerator-and-data-augmentation/

from keras.models import Model

from keras.models import clone_model   # Model cloning is similar to calling a model on new inputs, except that it creates new layers (and thus new weights) instead of sharing the weights of the existing layers.
...
import PIL.Image as Image, PIL.ImageDraw as ImageDraw, PIL.ImageFont as ImageFont   # PIL : the Python Imaging Library which provides the python interpreter with image editing capabilities.  https://www.geeksforgeeks.org/python-pil-image-new-method/
...
print(class_map_df.shape, sample_sub_df.shape)
...
top_10_roots
f, ax = plt.subplots(2, 5, figsize=(16, 8))   # fig, ax = plt.subplots()
...
for i in range(10):
    ...
    ax[i].imshow(image_from_char(top_10_roots['component'].iloc[i]), cmap='Greys')  # cmap : colormap
...
train_df = pd.merge(pd.read_parquet(f'/kaggle/input/bengaliai-cv19/train_image_data_0.parquet'), train_df_, on='image_id').drop(['image_id'], axis=1)
...
def plot_acc(his, epoch, title):
    ...
    plt.plot(np.arange(0, epoch), his.history['dense_4_accuracy'], label='train_vowel_acc')

    plt.plot(np.arange(0, epoch), his.history['dense_5_accuracy'], label='train_consonant_acc')

    

    plt.plot(np.arange(0, epoch), his.history['val_dense_3_accuracy'], label='val_root_acc')

    plt.plot(np.arange(0, epoch), his.history['val_dense_4_accuracy'], label='val_vowel_acc')

    plt.plot(np.arange(0, epoch), his.history['val_dense_5_accuracy'], label='val_consonant_acc')
...
df_test_img = pd.read_parquet('/kaggle/input/bengaliai-cv19/test_image_data_0.parquet'.format(0)) # test data 가져옴
