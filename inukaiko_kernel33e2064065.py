...
base_image_dir = os.path.join('..', "input",'aptos2019-blindness-detection','train_images')

retina_df = pd.read_csv(os.path.join('..', "input",'aptos2019-blindness-detection','train.csv'))

retina_df['path'] = retina_df['id_code'].map(lambda x: os.path.join(base_image_dir,

                                                         '{}.png'.format(x)))

from keras.utils.np_utils import to_categorical

retina_df['level_cat'] = retina_df['diagnosis'].map(lambda x: to_categorical(x, 1+retina_df['diagnosis'].max()))
...
train_df = raw_train_df.groupby(['diagnosis']).apply(lambda x: x.sample(75, replace = True)
...
base_image_dir = os.path.join('..',"input",'aptos2019-blindness-detection', 'test_images')

test_df = pd.read_csv(os.path.join('..',"input",'aptos2019-blindness-detection','test.csv'))

test_df['path'] = test_df['id_code'].map(lambda x: os.path.join(base_image_dir,
...
test_df['level_cat'] = test_df['diagnosis'].map(lambda x: to_categorical(x,5))
...
for i in range(test_df.shape[0]):
    ...
    image = Image.open(test_df['path'][i])

    img_resized = image.resize(IMG_SIZE)

    data = asarray(img_resized).reshape([1,512,512,3])

    res = np.argmax(retina_model.predict(data))
