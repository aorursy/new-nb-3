...
plt.show()
df1 = data[(data['Total Volume'] <= 1.071027e+05) & (data['type'] == 0)]

df2 = data[(data['Total Volume'] > 1.071027e+05) & (data['type'] ==0 )]

df3 = data[ (data['Total Volume'] > 1.071027e+05) & (data['type'] == 1)]

df4 = data[(data['Total Volume'] <= 1.071027e+05) & (data['type'] == 1)]
#rgr1
...
rgr1 = RandomForestRegressor(n_estimators=50)
...
x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size = 0.2, random_state=42)
rgr2 = RandomForestRegressor(n_estimators=50)
...
x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size = 0.2, random_state=42)
rgr3 = BaggingRegressor(n_estimators=50)
...
X = df4.drop(['AveragePrice','type'],axis=1)

Y= df4['AveragePrice']

x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size = 0.2, random_state=42)
rgr4 = RandomForestRegressor(n_estimators=50)

print (cross_val_score(rgr4, X, Y, cv=k_fold, n_jobs=1, scoring = mse))
rgr4.fit(x_train,y_train)

pred=rgr4.predict(x_test)
...
print(test.sample(2))
test.info()
tf1 = test[(test['Total Volume'] <= 1.071027e+05) & (test['type'] == 0)]

tf2 = test[(test['Total Volume'] > 1.071027e+05) & (test['type'] ==0 )]

tf3 = test[ (test['Total Volume'] > 1.071027e+05) & (test['type'] == 1)]

tf4 = test[(test['Total Volume'] <= 1.071027e+05) & (test['type'] == 1)]
...
s4['AveragePrice'] = rgr4.predict(tf4)
sol = [s1, s2, s3, s4]

solution = pd.concat(sol, ignore_index=True)

solution
#solution.to_csv('solnf_4models_totvol50%&typebased_WOtype_BagRegin3.csv', index=False)