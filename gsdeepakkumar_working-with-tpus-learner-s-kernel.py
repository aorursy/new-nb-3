...
len(CLASSES)
print(f'Length of training files:{len(TRAINING_FILENAMES)} testing files:{len(TEST_FILENAMES)} validation files:{len(VALIDATION_FILENAMES)}')
...
# NUM_TEST_IMAGES = count_data_items(TEST_FILENAMES)
#https://www.kaggle.com/xhlulu/aptos-2019-densenet-keras-starter - Added dropout
...
history = model.fit(get_training_dataset(), steps_per_epoch=STEPS_PER_EPOCH, epochs=EPOCHS, validation_data=get_validation_dataset(),callbacks=[es,rlr])
