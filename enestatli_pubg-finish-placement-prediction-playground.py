...
#sampleMid = fullplayer[0:5]
sampleMid = train['matchId'].unique()[0:5]
for mid in sampleMid:
...
minKills = all_data.sort_values(['matchId','groupId','kills','killPlace']).groupby(['matchId','groupId']).first().copy()
for n in [0,1,2]:
